...
def _evaluate_blocked(
    ...
    citations.append(
        build_policy_citation(
            f"policy.licenses.blocked[{license_id}]",
            f"{license_id} is in the organization blocked license list.",
        )
    )
    ...
    return _make_finding(
        ...
        description=(
            ...
            f"with this license per organization policy."
        ),
        recommendation=recommendation,
        remediations=remediations,
        citations=citations,
...
async def _evaluate_license(
    ...
    blocked = set(licenses_cfg.get("blocked") or [])
